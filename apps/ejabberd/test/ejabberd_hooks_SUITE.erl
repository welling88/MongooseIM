...
all() ->
    [{group, hook_and_handler_semantics}].

groups() ->
    [{hook_and_handler_semantics, [shuffle],
      [ a_fun_can_be_added,
...
a_fun_can_be_added(_) ->
    ...
    % then
    [{{test_run_hook,?HOST}, [{1,undefined,_}]}] = get_hooks().
...
a_module_fun_can_be_added(_) ->
    ...
    % then
    [{{test_run_hook,?HOST}, [{1,hook_mod,fun_a}]}] = get_hooks().
...
a_fun_can_be_removed(_) ->
    ...
    % then
    [{{test_run_hook,?HOST}, []}] = get_hooks().


a_module_fun_can_be_removed(_) ->
    given_hooks_started(),
    given_module(hook_mod, fun_nullary, fun() -> success0 end),
    given_hook_added(test_run_hook, ?HOST, hook_mod, fun_nullary, 1),
...
hooks_run_launches_unary_fun(_) ->
    ...
    given_hook_added(test_run_hook, ?HOST, hook_mod, fun_onearg, 1),
...
hooks_run_ignores_different_arity_funs(_) ->
    ...
    given_hook_added(test_run_hook, ?HOST, hook_mod, fun_onearg, 1),
    given_hook_added(test_run_hook, ?HOST, hook_mod, fun_twoarg, 1),
...
hooks_run_stops_when_fun_returns_stop(_) ->
    ...
    given_hook_added(test_run_hook, ?HOST, hook_mod, a_fun, 1),
    given_hook_added(test_run_hook, ?HOST, hook_mod, another_fun, 2),
...
hooks_run_fold_folds_with_unary_fun(_) ->
    ...
    given_hook_added(test_fold_hook, ?HOST, hook_mod, unary_folder, 1),
...
hooks_run_fold_folds_with_binary_fun(_) ->
    ...
    given_hook_added(test_fold_hook, ?HOST, hook_mod, binary_folder, 1),
...
hooks_run_fold_passes_acc_along(_) ->
    ...
    given_hook_added(test_fold_hook, ?HOST, hook_mod1, first_folder, 1),
    given_hook_added(test_fold_hook, ?HOST, hook_mod2, second_folder, 2),

    %% when
    R = ejabberd_hooks:run_fold(test_fold_hook, ?HOST, 0, [10]),

    ct:pal("~p", [mongoose_metrics:get_metric_values(?HOST)]),
...
hooks_run_fold_stops_when_fun_returns_stop(_) ->
    ...
    given_hook_added(test_fold_hook, ?HOST, hook_mod1, stopper, 1),
    given_hook_added(test_fold_hook, ?HOST, hook_mod2, folder, 2),
...
error_in_run_fold_is_ignored(_) ->
    ...
    given_hook_added(test_fold_hook, ?HOST, failing_mod, broken, 1),

    given_module(working_mod, good, const(i_was_run)),
    given_hook_added(test_fold_hook, ?HOST, working_mod, good, 2),
...
throw_in_run_fold_is_ignored(_) ->
    ...
    given_hook_added(test_fold_hook, ?HOST, throwing_mod, throwing_fun, 1),
...
exit_in_run_fold_is_ignored(_) ->
    ...
    given_hook_added(test_fold_hook, ?HOST, exiting_mod, exiting_fun, 1),
